...
public class TeacherUpdateTest {
    ...
    @Test
    void testSuccessfulTeacherUpdateWithDifferentData() {
        int idTeacher = -1;
        try {
            teacherForTesting.setEmail("Anónimo2@uv.mx");
            teacherForTesting.setName("Anónimo actualizado");
            teacherForTesting.setPassword("anonimo_contraseña");
            teacherForTesting.setStudyGrade("Maestría");
            idTeacher = TEACHER_DAO.updateTeacher(teacherForTesting.getId(), teacherForTesting);
        } catch (DAOException ex) {
            Logger.getLogger(TeacherRegistrationTest.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    @Test
    void testSuccessfulTeacherUpdateWithSameData() {
        int idTeacher = -1;
        try {
            idTeacher = TEACHER_DAO.updateTeacher(teacherForTesting.getId(), teacherForTesting);
        } catch (DAOException ex) {
    ...
    @Test
    void testFailedTeacherUpdateDueToEmailDuplication() {
        Assertions.assertThrows(DAOException.class, () -> {
            teacherForTesting.setEmail(DEFAULT_AUX_EMAIL);
            TEACHER_DAO.updateTeacher(teacherForTesting.getId(), teacherForTesting);
        });
    }
